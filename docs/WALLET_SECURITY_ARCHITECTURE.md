...
The verified identity, amount, nonce/fee, serialization, transaction-identifier, signature, submission-response, and receipt-observation contracts do not unlock user-facing transaction signing. Wallet creation and restore remain disabled until confirmation/finality presentation, history limitations, and private-loopback compatibility are approved and the recovery UX and independent security review pass.
