...
The recommended first-release source was approved on 2026-08-01. The internal version 2 journal records the transaction identifier, public sender and recipient, exact raw amount, nonce, tip, fee limit, timestamps, and validated receipt observations. It excludes signed bytes, signatures, vault credentials, signing seeds, recovery material, and plaintext secrets. Each event carries a domain-separated BLAKE3 tag keyed by a dedicated wallet-seed-derived subkey and the preceding event tag. Loading verifies wallet ownership, sender identity, event content, sequence, and chain order before exposing records. Corruption and authentication failure are indistinguishable and fail closed. The journal remains display metadata only and is never an authority for balances, nonces, signing, or transaction success. A complete rollback to an older authentic prefix remains possible until an external protected head anchor is designed.
