...
The supported wallet host boundary is standard Windows Client with one interactive session per
Windows account. Concurrent same-account Windows Server, Remote Desktop Services, Azure Virtual
Desktop multi-session, and other multi-session hosts are unsupported. The per-user `Global\`
wallet lease still spans Windows sessions as defense in depth and denies a second runtime, but that
