...
These primitives do not form a spending state machine. The required intent binding, trusted final
confirmation, session-bound signing, private Core client, ambiguous-outcome reconciliation,
receipt tracking, journal failure behavior, and eventual narrow Tauri boundary are specified in
`WALLET_TRANSACTION_AUTHORITY_BOUNDARY.md`. That specification grants no runtime authority and does
not permit command registration.

The activity journal schema is version 2. Every event is authenticated with a domain-separated BLAKE3 keyed hash using a dedicated subkey derived from the wallet seed, and every event authenticates the preceding event tag. A separate bounded head file uses another seed-derived key and a recoverable transition/commit protocol to bind the exact expected sequence and tag. Replacing the journal alone with an older authentic prefix, deleting the head, or interrupting either publication phase now fails closed or deterministically recovers the matching complete state. The journal verifies exact wallet identity, sequence, chain order, and event content before exposing local records; an accepted transaction must also have the sender address derived from the authenticating seed. Windows reads are handle-bound beneath a held non-reparse directory chain. Updates use protected, flushed, random create-new staging files and atomic handle-based replacement rather than modifying live files in place. Alternate data streams and reparse-point files fail closed. A restrictive per-user `Global\` Windows process lease excludes a second wallet runtime across console, fast-user-switching, and RDP sessions before it can read or replace journal state. The journal remains size-bounded, synchronized within the Desktop process, and stored with restrictive filesystem permissions. It records only public transfer metadata after an exact accepted submission and later validated receipt observations. It never stores signed bytes, signatures, passwords, seeds, recovery material, or vault contents, and it never supplies balances, nonces, or signing decisions. Coordinated rollback of both local authenticated files or a complete Windows profile snapshot remains a documented local-only limitation. Its `High confidence` presentation at 50 canonical confirmations is a Desktop diagnostic policy, never a claim of deterministic finality.
