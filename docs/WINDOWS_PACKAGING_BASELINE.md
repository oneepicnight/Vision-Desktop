...
- Production artwork qualification recorded: `2026-08-01T00:45:10-04:00`
- Repository: `C:\Vision\Vision-Desktop`
- Branch: `main`
- Source baseline before this packaging commit: `7c96d031414693a881dbb448003cf9473ba6745a`
- Source baseline before the production artwork commit: `e4013558ae827e32331c84bc557a76106c09b575`
...
## Production Windows Branding

The Windows bundle now uses deterministic Vision artwork generated by `scripts/generate_windows_branding.py`. The artwork preserves the code-native Desktop identity: a tilted cyan-to-violet `V` mark on a dark network-operations presentation.

| Asset | Dimensions / frames | Purpose | SHA-256 |
| --- | --- | --- | --- |
| `src-tauri/icons/icon.png` | 1024 x 1024 RGBA | High-resolution transparent application mark | `74A78B1CD4B569A2856A90DFEA3C818257A27326F5390ECF12FAB4745259959A` |
| `src-tauri/icons/icon.ico` | 16, 24, 32, 48, 64, 128, and 256 px | Windows executable, installer, uninstaller, and shortcut icon | `B516F09756906692FD68A1F662BAC1274351E24E71B2FE67B5D3C56209A0B795` |
| `src-tauri/icons/nsis-sidebar.bmp` | 164 x 314 RGB | NSIS completion-page Vision artwork | `F4AB86F7E9DE309FF928C2AE86087EFE731AC1C3B97803742DBE7289AC021E61` |
| `src-tauri/icons/nsis-header.bmp` | 150 x 57 RGB | Cyan-accented installer header | `743727511012239531075751115BE77CFF6AE1B119361C14DECCC2542F74AB93` |
| `src-tauri/icons/nsis-uninstaller-header.bmp` | 150 x 57 RGB | Coral-accented uninstaller header | `79C283C67C6CFC2D6BB4C68B99FE5B14C179DA5D3DA4D020BE36784DA40247F6` |

`src-tauri/tauri.conf.json` maps these assets explicitly through the Tauri 2 NSIS configuration and fixes the NSIS installation scope to `currentUser`. Regenerating the assets is repeatable and does not require a design dependency or network service.

## Artifacts

| Artifact | Size | SHA-256 | Authenticode |
| --- | ---: | --- | --- |
| `src-tauri/target/release/vision-desktop.exe` | 12,158,976 bytes | `2E752C3138178A23F19DD743BC32EB9C8313E34B6F5063B80BBF384D74E2A4CA` | Not signed |
| `src-tauri/target/release/bundle/msi/Vision Desktop_0.1.0-alpha.1_x64_en-US.msi` | 6,520,832 bytes | `7AFBE2BD99480CA9E4F9516BA5A1171C7D7C5A60E2E64ECA84590FC2B3A3D938` | Not signed |
| `src-tauri/target/release/bundle/nsis/Vision Desktop_0.1.0-alpha.1_x64-setup.exe` | 4,283,643 bytes | `953F6F3DB2ED3021C854B64586C7119C79912D6CB86BE09EA142508CAF8F4C08` | Not signed |
...
The interactive NSIS lifecycle is functionally qualified on this workstation. This qualification predated the production artwork; the branded lifecycle below supersedes its presentation findings. Clean-machine compatibility, upgrades, downgrades, signing, and public distribution are not qualified by this result.

## Production Artwork and Branded Lifecycle Qualification

Recorded: `2026-08-01T00:45:10-04:00` on the same ASUS Windows workstation.

The rebuilt production-artwork package passed a complete visible current-user lifecycle:

- the installer opened on the `Choose Install Location` page and rendered the cyan Vision header correctly;
- the installation-progress and completion pages retained the Vision header and completed successfully;
- the final page rendered the full dark Vision sidebar artwork and offered explicit checked options to run Vision Desktop and create a Desktop shortcut;
- after explicit approval, the installed application launched from `C:\Users\bighe\AppData\Local\Vision Desktop\vision-desktop.exe`;
- the packaged application window used the Vision icon and rendered the complete Node Manager layout, navigation, and development mock snapshot correctly;
- the uninstaller rendered the coral Vision header and showed the exact per-user installation directory;
- `Delete the application data` remained unchecked during the approved test uninstall;
- uninstall removed the program directory, host-user uninstall registration, Start menu shortcut, Desktop shortcut, and running process;
- `C:\Users\bighe\AppData\Local\com.vision.desktop` remained, confirming that application data was retained as selected.

The production icon and NSIS artwork are visually qualified on this workstation. This result does not qualify clean-machine behavior, accessibility across every Windows display scale, upgrades, downgrades, code signing, or public distribution.

## Confirmed Packaging Fixes

- Added the explicit Windows icon required by the Tauri bundle step.
- Replaced the preliminary icon with a production multi-resolution Windows icon set.
- Added deterministic source generation for the application icon and NSIS artwork.
- Configured separate branded NSIS installer and uninstaller headers plus the completion-page sidebar.
...
- Review branded installer accessibility and presentation across supported Windows display scales.
